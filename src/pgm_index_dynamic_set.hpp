...
namespace pgm {
  ...
  class DynamicPGMIndexSet {
    ...
    public:
       ...
       */
      DynamicPGMIndexSet(uint8_t base = 8, uint8_t buffer_level = 0, uint8_t index_level = 0)
        : base(base),
       ...
       */
      void insert_or_assign(const K &key) { insert(Item(key)); }

      /**
       * Removes the specified element from the container.
       * @param key key value of the element to remove
       */
      void erase(const K &key) { insert(Item(key, true)); }
  ...
  class DynamicPGMIndexSet<K, PGMType>::Iterator {
    ...
    using dynamic_pgm_type = DynamicPGMIndexSet<K, PGMType>;
